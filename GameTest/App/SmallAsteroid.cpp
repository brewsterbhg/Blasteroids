...
{
public:
	static const float SM_VEL;
};

//Set up float consts
const float MY_CONSTS::SM_VEL = 3.5;

void SmallAsteroid::New(float direction, float x, float y)
{
	...
	//Set the HP
	m_health = SMALL_ASTEROID_HEALTH;
...
{
	if (m_isActive) {
		float firstPointX = _x - 20;
		float firstPointY = _y + 20;
		float secondPointX = _x - 25;
		float secondPointY = _y + 5;
		float thirdPointX = _x - 25;
		float thirdPointY = _y - 10;
		float fourthPointX = _x - 5;
		float fourthPointY = _y - 10;
		float fifthPointX = _x - 10;
		float fifthPointY = _y - 20;
		float sixthPointX = _x + 5;
		float sixthPointY = _y - 20;
		float seventhPointX = _x + 20;
		float seventhPointY = _y - 5;
		float eighthPointX = _x + 5;
		float eightPointY = _y + 5;
		float ninthPointX = _x + 20;
		float ninthPointY = _y + 10;
		float tenthPointX = _x + 10;
		float tenthPointY = _y + 20;
...
void SmallAsteroid::Update()
{
	...
	m_trajectoryX = MY_CONSTS::SM_VEL * sin(m_direction * (PI / 180));
	m_trajectoryY = MY_CONSTS::SM_VEL * cos(m_direction * (PI / 180));

	//Move in the direction the asteroid is facing
	_x += m_trajectoryX;
	_y += m_trajectoryY;

	CheckForWrap(_x, _y, SM_WIDTH / 2);
}

void SmallAsteroid::AsteroidHit(int numOfHits)
{
	...
	for (int i = 0; i < numOfHits; i++)
	{
		...
		if (m_health == SMALL_ASTEROID_HEALTH - 1)
		{
			_G = 0.0;
			_B = 0.0;
			_R = 1.0;
...
void SmallAsteroid::DestroyAsteroid()
{
	m_isActive = false;
}

bool SmallAsteroid::CheckCollision(float objectX, float objectY, float radius)
{
	bool collide = false;
...
bool SmallAsteroid::GetIsActive()
{
	return m_isActive;
}